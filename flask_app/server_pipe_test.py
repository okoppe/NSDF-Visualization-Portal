...
LOCAL_GITHUB_REPO = os.environ["LOCAL_GITHUB_REPO"] 

print("SERVER_IP",SERVER_IP)
print("GITHUB_REPO",GITHUB_REPO)
print("LOCAL_GITHUB_REPO",LOCAL_GITHUB_REPO)
...
class jupterNotebook:
	...
	def serveBokehApp(self): 
		def startServer(self):
			BOKEH_ALLOW_WS_ORIGIN=SERVER_IP+':'+str(self.port)
			subprocess.call(['python3', '-m', 'bokeh', 'serve',  self.filePath, '--port', str(self.port), 
				 '--allow-websocket-origin='+SERVER_IP+':'+str(self.port)])
	...
	def getPortLink(self):
		return ('http://'+SERVER_IP+':'+str(self.port)+'/'+self.fileName.replace(".ipynb",""))
	...
	def updateLocalFiles(self):
		...
		for newFile in thisPullFiles:
			if newFile not in self.fileArray:
				...
				jnb = jupterNotebook(newFile, LOCAL_GITHUB_REPO+newFile, port)
				jnb.serveBokehApp()
...
def f(child_conn):
	global j1
	if j1 is None:
		j1 = jupterNoteBookList(GITHUB_REPO, LOCAL_GITHUB_REPO)
		j1.loopUpdate()
